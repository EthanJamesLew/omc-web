...
def _find_headers() -> Path:
    ...
    if env:
        return Path(env)
    candidates = []
    omb = os.environ.get("OMBOOTSTRAPPING")
    if omb:
        candidates.append(Path(omb) / "bootstrap-sources" / "build")
    omc = os.environ.get("OMC_ROOT")
    if omc:
        candidates.append(Path(omc) / "OMCompiler" / "Compiler" / "boot" / "bootstrap-sources" / "build")
    for c in candidates:
        if c.is_dir():
            return c
    raise RuntimeError(
        "no OMC bootstrap-sources/build directory found "
        "(set OMBOOTSTRAPPING or OMC_ROOT env var)")

HEADERS_DIR = _find_headers()
SHIMS_DIR = Path(os.environ.get("SHIMS_DIR", str(ROOT / "common-shims")))
STUB_FILE = SHIMS_DIR / "omcweb_stubs_auto.c"
HAND_FILE = SHIMS_DIR / "omcweb_stubs.c"
...
def _symbols_in_archive(ar: Path) -> set[str]:
    ...
    Missing archive or emnm — return empty set (no dedup that round)."""
    if not ar.exists():
        return set()
    import subprocess
    try:
        ...
        print("[gen-stubs] WARN: emnm not on PATH — skipping archive dedup", file=sys.stderr)
        return set()
    if r.returncode != 0:
        return set()
    out = set()
