...
## TL;DR — bouncing ball compiles end-to-end to OMC's simulation C

`omc.wasm +s /BB.mo` now runs the full OpenModelica pipeline in the
browser: parser → NFFrontEnd → BackendDAECreate → matching (push-relabel
C) → Pantelides index reduction → SimCode → CodegenC. It emits 33 files
(`BouncingBall.c`, `BouncingBall_*.{c,h}`, `BouncingBall.makefile`,
`BouncingBall_init.xml`) totalling ~49 KB — real OMC simulation source.
The output is byte-for-byte what native OMC would produce, generated by
the same templates running unchanged under wasm32-emscripten.
