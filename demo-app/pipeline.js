...
// All asset URLs are resolved relative to document.baseURI so the site
// works both at root (local serve) and at a project-pages subpath such as
...
async function getEmception(sink) {
  ...
  emcePromise = (async () => {
    ...
    const worker = new Worker(assetUrl("emception/emception.worker.bundle.worker.js"));
...
async function stageEmceptionAssets(em, sink) {
  ...
  for (const f of SYSLIBS) await stageOnce(em, assetUrl("emscripten-libs/" + f), "/working/" + f);

  sink("info", `staging ${BB_LIBS.length} sim libs`);
  for (const f of BB_LIBS) await stageOnce(em, assetUrl("model-bundle/libs/" + f), "/working/" + f);

  sink("info", "staging omcweb_gc_stub.o");
  await stageOnce(em, assetUrl("model-bundle/objs/omcweb_gc_stub.o"), "/working/omcweb_gc_stub.o");

  sink("info", "unpacking headers.zip");
  const zipBuf = await (await fetch(assetUrl("headers.zip"))).arrayBuffer();
